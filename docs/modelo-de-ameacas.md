...
| AM-21 | Pipeline CI | Tampering | Action de terceiro comprometida (tag movida ou pacote sequestrado) executa código arbitrário no runner | Toda action pinada por SHA completo; a configuração do repositório recusa a execução de qualquer action não pinada, e o CodeQL para `actions` bloqueia o merge de action de terceiros não pinada | Implementado (`SUP-01`) |
